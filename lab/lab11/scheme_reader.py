...
def scheme_read(src):
    ...
    """
    "read anything exclude ')' !"
    if src.current is None:
        raise EOFError
    val = src.pop_first()  # Get and remove the first token (in each read_loop)
    if val == 'nil':
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        return nil
        # END PROBLEM 2
    elif val == '(': # this is opening parenthesis.
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        ...
        # it is a Scheme list (as with all special forms).
        return Pair('quote', Pair(scheme_read(src), nil))
        # END PROBLEM 3
    elif val not in DELIMITERS:
        return val # return the the first token to build Pair in scheme_read() or read_tail()
    else:
...
def read_tail(src):
    ...
    try:
        ...
        if src.current is None:
        ...
        elif src.current == ')': # this is closing parenthesis.
            # BEGIN PROBLEM 2
            "*** YOUR CODE HERE ***"
            src.pop_first()
            return nil
            ...
            # which consists of two mutually recursive functions: scheme_read and read_tail. 
            # Each function takes in a single src parameter, which is a Buffer instance.
...
#  Shorter version of scheme_read
def read_line(line):
