...
def calculator_ops(calc_str):
    ...
    False
    """
    return bool(re.search(r'(\([\+\-\*\/\s0-9]{5}\))+', calc_str))
...
def roman_numerals(text):
    ...
    False
    """
    # my solution has error, because it not include all situations and it just passes test  
    return bool(re.search(r'[I]{1,3}[|V]|V[I]{0,5}', text))
