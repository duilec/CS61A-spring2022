...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        if player.hand:
            added = True
            player.hand.append(player.hand[0].copy()) # class of card has a method of copy()
        # losing current round: Tutor power is -float('inf')(i.e. -oo)
        # END Problem 4
        # You should add your implementation above this.
        if added:
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if player.hand:
            ...
            for i in range(len(player.hand)):
                player.hand[i].power(opponent_card) > player.hand[maxpow_i].power(opponent_card)
                maxpow_i = i
            self.attack += player.hand[maxpow_i].attack
            self.defense += player.hand[maxpow_i].defense
            best_card = player.hand.pop(maxpow_i) #best_card is a card
        # END Problem 5
        if best_card:
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        # each round sub 1000 whether it's alive or not after checking
        self.attack -= 1000
        self.defense -= 1000
        # END Problem 6
        # You should add your implementation above this.
