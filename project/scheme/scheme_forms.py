...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        # expressions.rest.first is builtin procedure either num
        ...
        env.bindings[name] = define_lambda
        return signature.first
        # you also can use line 56-57 instead of line 52
        # validate_formals(formals)
        # define_lambda = LambdaProcedure(formals, body, env)
        # END PROBLEM 10
    else:
...
def do_if_form(expressions, env):
    ...
    # tail call: Sub-expressions 2 & 3 in a tail context 'if' expression
    if is_scheme_true(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env, True) # tail call: ret Unevalated 
    elif len(expressions) == 3:
        return scheme_eval(expressions.rest.rest.first, env, True) # tail call: ret Unevalated 
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            "*** YOUR CODE HERE ***"
            if clause.rest == nil:
                return test # just ret 'test', you can't test again
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        # check size of sub_bindings
        # assignment(let) only has 2 parts: left is name, right is a val or symbol or expr
        validate_form(sub_bindings, 2, 2)
        names = Pair(sub_bindings.first, names)
        # if 'sub_bindings.rest.first'(right) is a symbol or expr we evaluate its value
        vals = Pair(scheme_eval(sub_bindings.rest.first, env), vals) 
        bindings = bindings.rest
    # you can't use same name in names
...
def do_define_macro(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    # check: expressions is a list of length at least 2
    validate_form(expressions, 2)
    
    # check: it is Scheme list?
    signature = expressions.first
    if not scheme_listp(signature):
        raise SchemeError("it is not Scheme list")
    
    # check: it is pass number?
    name = signature.first
